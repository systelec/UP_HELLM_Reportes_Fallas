<template>
  <v-container>
    <v-layout row class="filtro">
      ...
      <v-flex style="height: 60px; width: 150px; border-left: 1px solid grey">
        <v-text-field
          v-model="minimoTiempo"
          class="ml-2"
          hide-details
          single-line
          label="Minimo tiempo [min]"
          placeholder="Minimo tiempo [min]"
          type="number"
        ></v-text-field>
      </v-flex>
      <v-flex class="mt-4">seg</v-flex>
      <v-flex>
...
export default {
  ...
  created() {
    ...
    this.SET_MINIMO_TIEMPO(this.minimoTiempo)
  },
  methods: {
    ...mapMutations([
      'SET_DESDE',
